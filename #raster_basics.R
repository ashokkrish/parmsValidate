...
selectedCountry <- "Nigeria" #"Korea" # "Czech Republic" # "Israel" # 

rasterAgg <- 15 # 25 #10 # 5 # 
...
cellStats(WorldPop == 0, sum) # Number of grid cells that are have a population count = 0

cellStats(!is.na(WorldPop), sum) # Number of grid cells that are have a population count

cellStats(is.na(WorldPop), sum) # Number of grid cells that are NA

cellStats(WorldPop, sum) # Total estimated population count
...
# 
 createBasePlot(selectedCountry = selectedCountry, rasterAgg = 0, directOutput = FALSE) # Use this for JMM
